...
- **รอบแยก model/part_name + รีพอร์ทไม่บังคับ (4 ส.ค. 2026):** ช่องชิ้นงานเดิม (เคยพิมพ์รวมเป็นก้อนเดียวเช่น "P703 LED HL HG") แยกเป็น 2 ช่อง — **Model** ("P703") กับ **ชื่อชิ้นงาน** ("LED HL HG") — ทั้งบน `request_parts` และ cache บน `test_items` · เพิ่ม **วันที่ requester อยากได้ผล** (ไม่การันตี) และ **checkbox "ต้องการรีพอร์ทไหม"** ระดับใบ (default ต้องการ) ที่หน้าลงงานใหม่ — ถ้าไม่ติ๊ก ทุก item ในใบข้ามเงื่อนไขบังคับกรอกรีพอร์ทตอนเข้าสถานะ 7 ได้เลย (field `report_required` บน `test_requests`)
- **Deploy:** ทดลองขึ้น **Railway สำเร็จแล้ว** (`docs/แผน-ทดลอง-railway-free.md` มีผลจริง + กับดัก 5 ข้อ · พิสูจน์แล้วว่า volume เก็บข้อมูลข้าม deploy ได้) · GitHub auto-deploy ทำงานอยู่ (push main → deploy อัตโนมัติ) · **โค้ด Phase 4 ขึ้น Railway แล้วและตรวจสอบแล้วจริง (3 ส.ค. 2026)** — migration รันผ่านอัตโนมัติ (`No pending migrations to apply`), ทดสอบ login หน้าจริงบน production URL สำเร็จ, เห็นหน้าแรกแบบ role-based ใหม่ · ทางเลือกที่แนะนำระยะยาว = **NAS + Cloudflare Tunnel** (`docs/แผน-ย้ายไป-nas-cloudflare.md` — ติดที่ยังไม่รู้ว่า NAS รุ่นอะไร ต้องทำ Phase 0 ก่อน)
- ขั้นถัดไปอื่น (backlog): **ยังไม่มี user LAB_HEAD/DEPT_HEAD จริงในระบบ** — ต้องสร้างที่ `/settings/users` แล้วมอบหมายหัวหน้าแผนกที่ `/settings/approvals` ก่อนใช้ flow อนุมัติได้จริง · เปิด groundwork equipment/test method, ตั้ง cron แจ้งเตือนจริง (ตอนนี้แจ้งเตือนยิงตอนมีคนเปิดหน้าเท่านั้น) — รวมทั้งหมดใน `docs/โครงสร้างโปรเจค-webapp.md`
...
test_requests (ใบรีเควส)   ใครขอ (ชื่อ/อีเมล/เบอร์) · วันที่รับใบ · test_object (ส่งอะไรมา) · purpose (ทำไม) · แผนก · created_by
                            · desired_date (วันที่ requester อยากได้ผล — ไม่การันตี) · report_required (ต้องการรีพอร์ทไหม default true)
   ├─ request_parts        ชิ้นงาน/รุ่น Lamp ที่ส่งมา — 1 ใบมีได้หลายรุ่น (model เช่น "P703" · part_name เช่น "LED HL HG" · part_no · จำนวน)
   └─ test_items           รายการทดสอบ — 1 ใบมีได้หลายรายการ · แต่ละรายการมี สถานะ/แผน/ผู้รับผิดชอบ/รีพอร์ท/QR ของตัวเอง
         ↕ (หลาย-ต่อ-หลาย) เลือกว่าทดสอบรุ่นไหนบ้าง — model/part_name/part_no/qty ใน item เป็น cache ที่สรุปจากรุ่นที่เลือก
...
กติกาบังคับกรอก: เข้าสถานะ 3 ต้องมีวันที่รับพาร์ท+ตำแหน่งเก็บ | เข้า 7 ต้องมีวันที่ส่ง + (ลิงก์รีพอร์ท หรือ บันทึกว่าส่งให้ใคร) — ข้ามเงื่อนไขนี้ได้ทั้งหมดถ้าใบนี้ตั้ง `report_required=false` | เข้า 8 ต้องมีที่เก็บชิ้นงาน+ที่เก็บ raw data
