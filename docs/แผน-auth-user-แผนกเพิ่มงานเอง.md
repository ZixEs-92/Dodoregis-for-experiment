...
> เอกสารแผน (Phase 3) · อัปเดต 2026-07-31 — **3a + 3b + 3e + 3c ทำแล้ว** · เหลือ **3d (คิวรอ admin วางแผน)** อย่างเดียว
...
**ความคืบหน้า (2026-07-31): 3a + 3b + 3e + 3c ทำแล้ว** — login/session (bcrypt + jose cookie 8ชม.), 4 roles, กันสิทธิ์ฝั่ง server ทุก action + page guard + ซ่อน UI ตาม role · **จัดการผู้ใช้ผ่านหน้าเว็บ `/settings/users`** (สร้าง/ตั้งรหัส/ปิดบัญชี — ไม่ต้องพิมพ์คำสั่งแล้ว, ยังมี `npm run create-user` สำรอง) · **requester portal**: `ownerId` nullable, แผนกล็อกฝั่ง server, เห็นเฉพาะแผนกตัวเอง, ช่อง owner/plan ซ่อน (admin วางแผนให้), แสดง "ยังไม่มอบหมาย" ทุกหน้า, `changedBy` เติมจาก session · หน้า login เป็นประตูทางเข้า (สแกน QR / เข้าดูไม่ล็อกอิน) · **เหลือ: 3d (คิวรอ admin วางแผน)**
